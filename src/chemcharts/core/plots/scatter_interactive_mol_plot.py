...
class ScatterInteractiveMolPlot(BasePlot):
    def __init__(self):
        super().__init__()

    def plot(self, chemdata_list: List[ChemData], parameters: dict, settings: dict):
        # base class call
        super(ScatterInteractiveMolPlot, self).plot(chemdata_list, parameters, settings)

        # checks whether there is a score input
        value_input_result = _check_value_input(chemdata_list, "Scatter_Interactive_Mol")
        ...
        if value_input_result:      # checks whether _check_value_input function returns 'True'
            ...
            scatter_df = pd.DataFrame({_PLE.UMAP_1: chemdata_list.get_embedding().np_array[:, 0],
                                       ...
                                       "SMILES": list(chemdata_list.get_smiles()),
            ...
            fig = px.scatter_3d(scatter_df,
                                ...
                                title=parameters.get(_PE.PARAMETERS_PLOT_TITLE,
                                                     "Scatter Interactive Mol ChemCharts Plot")
                                )
